...
#import wi_functions
spec = ilu.spec_from_file_location("wi_functions",os.path.join(os.path.dirname(__file__), "metadata-organizer", "src", "wi_functions.py"))
...
def genConditions(data):
    new_data = metadata_organizer.get_conditions(data["factor_list"], data["organism_name"])
    return new_data
...
def finishResult(data, username):
    filenames = []
    file_name_to_save = username+"_" +datetime.now().strftime("%d_%m_%Y")
    save_path = ""
    if not data["edit"]:
        save_path = data["path"] + username + "/tmp/"
    else:
        save_path = data["path"] 
        
    metadata_object = metadata_organizer.parse_object(data["object"])
    filename_metadata, project_id = metadata_organizer.save_object(metadata_object, save_path, file_name_to_save)
    filenames.append(filename_metadata.split("/")[-1])
    #save wetlab.xlsx
    if not data["edit"]:
        all_exp_settings = sum(wetlab_writer.format_generator(wetlab_writer.find_keys(metadata_object, "experimental_setting")), [])
        ...
        filename_wetlab = wetlab_writer.write_wetlab(exp_setting_list_new, save_path, file_name_to_save_wetlab)
        filenames.append(filename_wetlab.split("/")[-1])
        move_process = threading.Thread(target=moveMetadata, args=(save_path, filenames,))
        move_process.start()
...
def moveMetadata(save_path,filenames):
    ...
    dest_dir = os.path.join(os.path.dirname(__file__), "user_data", "saved_metadata")
...
def createFilelist(data, username):
    path = os.path.abspath(os.path.join(os.path.dirname(__file__),"user_data", username, "tmp"))
