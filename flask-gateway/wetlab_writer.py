...
def format_sample_names_search(search_result_samples, search_result_filenames):
    ...
    for sample in sample_id_ls:
        sample_dict[sample] = []
        for sample_fullname, sample_filename in zip(sample_fullnames_ls,
                                                    sample_filenames_ls):
...
def format_conditions(all_conditions, keys):
    condition_dict = {}
    # new level for organism key because of exp setting
    for condition in all_conditions:
        condition_dict[condition["condition_name"]] = {}
        for key in keys:
            if key == "sample_name":
                condition_dict[condition["condition_name"]]["samples"] = \
                    format_sample_names_search(
                        list(find_keys(condition, key)),
                        list(find_keys(condition, "filenames")))
            else:
                condition_dict[condition["condition_name"]][key] = list(
...
def write_wetlab(exp_setting_dict, filepath, filename):
    ...
    blue_fill = PatternFill(start_color='B6B6B6', end_color='B6B6B6',
                            fill_type='solid')
    user_fill = PatternFill(start_color='F4A460', end_color='F4A460',
                            fill_type='solid')
    align_center = Alignment(horizontal='center', vertical='center')
    bold_font = Font(bold=True)
    for exp_setting_id in exp_setting_dict:
        
        exp_setting = exp_setting_dict[exp_setting_id]
        ws = wb.create_sheet(title=f'{exp_setting_id}')
        
        headers = ['sample_ID', 'condition', 'organism_name']
        additional_headers = []
        for col, header in enumerate(headers, start=1):
            ws.cell(row=2, column=col).value = header
            ws.cell(row=2, column=col).border = Border(bottom=Side(
                style='medium'))
        ...
        for sample_id, sample_data in exp_setting.items():
            for factor, factor_data in sample_data["factors"].items():
                if factor not in additional_headers:
                    ...
                    ws.cell(row=2, column=col_num).value = f'factor.{factor}'
        ...
        ws.cell(row=2, column=col_num).border = Border(bottom=Side(
            style='medium'))
        ws.cell(row=2, column=col_num).font = bold_font
        ws.cell(row=2, column=col_num).alignment = align_center
        col_num += 1   
        additional_headers_map['technique'] = col_num
        ws.cell(row=2, column=col_num).value = "technique"
        ws.cell(row=2, column=col_num).border = Border(bottom=Side(
            style='medium'))
        ws.cell(row=2, column=col_num).font = bold_font
        ws.cell(row=2, column=col_num).alignment = align_center
        ...
        # Fill the table with data for each sample
        for sample_id, sample_data in sorted(exp_setting.items()):
            ...
            ws.cell(
                row=row_num,
                column=additional_headers_map["sample_filename"]).value = \
                sample_data["sample_filename"]
            ws.cell(
                row=row_num,
                column=additional_headers_map['technique']).value = \
                sample_data['technique']
            ws.cell(
                row=row_num,
                column=additional_headers_map["sample_label"]).value = \
                sample_data["sample_label"]
            ws.cell(
                row=row_num,
                column=additional_headers_map["condition_label"]).value = \
                sample_data["condition_label"]
            ws.cell(
                row=row_num,
                column=additional_headers_map["tube_label"]).value = row_num-2

            for factor, factor_data in sample_data["factors"].items():
                ws.cell(
                    row=row_num,
                    column=additional_headers_map[factor]).value = \
                    str(factor_data)
        ...
        align_center_wrap = Alignment(horizontal='center', vertical='center',
                                      wrap_text=True)
        ...
        for category_title, category_data in wetlab_config.items():
            ...
            ws.cell(row=1, column=col_num).border = Border(left=Side(
                style='medium'))
            ...
            for row in range(3, row_num):
                ws.cell(row=row, column=col_num).border = Border(left=Side(
                    style='medium'))
            ...
            for i, header_object in enumerate(category_data["header_objects"]):
                ...
                if header_object["validation_type"] == "between":
                    ...
                    validation_between = DataValidation(
                        type="decimal", operator="between",
                        formula1=min_value, formula2=max_value,
                        errorStyle="stop", errorTitle="Invalid value entered",
                        showErrorMessage=True)
                    ws.add_data_validation(validation_between)
                    for row in range(3, row_num):
                        validation_between.add(ws.cell(row=row,
                                                       column=col_num))
                if header_object["validation_type"] == "formula":
                    min_value, max_value = header_object["validation_values"]
                    validation_between = DataValidation(
                        type="decimal", operator="between", formula1=min_value,
                        formula2=max_value, errorStyle="stop",
                        errorTitle="Invalid value entered",
                        showErrorMessage=True)
                    ws.add_data_validation(validation_between)
                    for row in range(3, row_num):
                ...
                # unlock user cells
                for row in range(3, row_num):
                    ws.cell(row=row, column=col_num).protection = Protection(
                        locked=False)
                col_num += 1
        ...
        for col_index, col in enumerate(ws.columns):
            ...
            for cell in col:
                try:
                    if len(str(cell.value)) > max_length and col_index+1 < \
                            user_columns:
                        # fixed size for all non-user cells
                        max_length = 11  # len(cell.value)
                        # small size for all numbers
                        if col_index+1 == user_columns-1:
                            max_length = 4
                        # bigger size for condition and sample name / remove
                        # protection
                        if col_index+1 == user_columns-2 or col_index+1 == \
                                user_columns-3:
                            max_length = 18
                            if col_index+1 == user_columns-3:
                                ...
                                # only a-z A-Z 0-9 - _
                                man_formula += \
                                    f'ISNUMBER(SUMPRODUCT(SEARCH(MID(' \
                                    f'{col[col_index+1].column_letter}' \
                                    f'3,ROW(INDIRECT(\"1:\"&LEN(' \
                                    f'{col[col_index+1].column_letter}' \
                                    f'3))),1),\"0123456789abcdefghijklmnop' \
                                    f'qrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_' \
                                    f'-\"))),'
                                ...
                                man_formula += \
                                    ...
                                    f'{col[col_index+1].column_letter}' \
                                    f'3,FIND(\"_\",' \
                                    f'{col[col_index+1].column_letter}3)+1)),'

                                # 1st character no digit
                                man_formula += \
                                    f'ISERROR(LEFT(' \
                                    f'{col[col_index+1].column_letter}3,1)*1),'
                                ...
                                # at least one _
                                man_formula += \
                                    f'NOT(ISERROR(FIND(\"_\",' \
                                    f'{col[col_index+1].column_letter}3))),'
                                ...
                                man_formula += \
                                    ...
                                    f'{col[col_index+1].column_letter}' \
                                    f'3))),1),\"0123456789abcdefghijklmnop' \
                                    f'qrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' \
                                    f'-\"))),'
                                ...
                                man_formula += \
                                    ...
                                    f'{col[col_index+1].column_letter}3))'
                                man_formula += ")"
                                # man_formula="=AND(ISERROR(FIND(\"_\"," +
                                # col[col_index+1].column_letter+"3)),
                                # ISERROR(FIND(\".\","+col[col_index+1].
                                # column_letter+"3)),ISERROR(LEFT("+
                                # col[col_index+1].column_letter+"3,1)*1))"
                                # man_formula=""
                                error_message = "bug\n1.) Do not start with " \
                                                "digit\n2.) Only use " \
                                                "alpha-numeric characters\n" \
                            ...
                            # print(man_formula)
                            validation_formula = DataValidation(
                                type="custom", formula1=man_formula,
                                errorStyle="stop",
                                errorTitle="Invalid value entered",
                                showErrorMessage=True, error=error_message)
                            ws.add_data_validation(validation_formula)
                            for row in range(3, row_num):
                                validation_formula.add(ws.cell(
                                    row=row, column=col_index+1))
                                ws.cell(
                                    row=row, column=col_index+1).protection = \
    ...
    ws2.cell(row=1, column=1).value = "Please provide pairwise comparisons " \
                                      "(contrasts). Keep in mind that " \
                                      "direction of log2FC is ConditionA / " \
                                      "ConditionB (please remove examples)"
    ...
    dxf = DifferentialStyle(font=Font(bold=True), fill=PatternFill(
        start_color='EE1111', end_color='EE1111'))
    rule = Rule(type="duplicateValues", dxf=dxf, stopIfTrue=None)
   
    for col in range(2):
        ws2.cell(row=2, column=col+1).border = Border(bottom=Side(
            style='medium'))
        ws2.cell(row=2, column=col+1).font = bold_font
        ws2.cell(row=2, column=col+1).alignment = align_center
        ws2.column_dimensions[ws2.cell(
            row=1, column=col+1).column_letter].width = 18
    cond_col = ws2.cell(row=1, column=user_columns-2).column_letter
    man_formula = f'={ws.title}!${str(cond_col)}$3:${str(cond_col)}$' \
                  f'{str(row_num-1)}'
    validation_list = DataValidation(
        type="list", formula1=man_formula, errorStyle="stop",
        errorTitle="Please choose valid condition from List",
        showErrorMessage=True)
    ...
    for row in range(20):
        ...
        srange = f'A{str(row+3)}:B{str(row+3)}'
...
def format_exp_settings(exp_data):
    ...
    for setting in exp_data:
        this_setting = {
            "organism": list(find_keys(setting, "organism_name"))[0],
            "conditions": format_conditions(sum(
                list(find_keys(setting, "conditions")), []), ["sample_name"]),
            'setting_id': list(find_keys(setting, 'setting_id'))[0]}
        exp_set_list.append(this_setting)
    return exp_set_list


def rename_exp_list(exp_setting_list):
    new_exp_setting_dict = {}
    for exp_setting in exp_setting_list:
        ...
        for condition in exp_setting["conditions"]:
            for sample_name in exp_setting["conditions"][condition]["samples"]:
                ...
                for single_sample, sample_filename in list(
                        exp_setting["conditions"][condition]["samples"][
                            sample_name]):
                    split_name = sample_filename.split('__')
                    if len(split_name) == 5:
                        project_id, unique_sample_index, used_technique, \
                            value_list, sample_index = split_name
                    else:
                        project_id, unique_sample_index, value_list, \
                            sample_index = split_name
                        used_technique = ''
                    sample_label = value_list + "_" + sample_index
                    condition_label = value_list
                    new_exp_setting[single_sample] = \
                        {"condition": "", "organism_name": "",
                         "sample_label": "", "condition_label": ""}
                    new_exp_setting[single_sample]["condition"] = condition
                    new_exp_setting[single_sample]["organism_name"] = \
                        exp_setting["organism"]
                    new_exp_setting[single_sample]["sample_label"] = \
                        sample_label
                    new_exp_setting[single_sample]["condition_label"] = \
                        condition_label
                    new_exp_setting[single_sample]["sample_filename"] = \
                        sample_filename
                    new_exp_setting[single_sample]["factors"] = factor_dict
                    new_exp_setting[single_sample]['technique'] = \
                        used_technique
                   
        new_exp_setting_dict[exp_setting['setting_id']] = new_exp_setting
    return new_exp_setting_dict
