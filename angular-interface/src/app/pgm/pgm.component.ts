...
export class PgmComponent implements OnInit {
  ...
  constructor(
    ...
    this.navService.initDataLoading('pgm').then(() => {
      this.original_exp_setting = JSON.parse(
        JSON.stringify(
          this.apiService.empty_pgmMask.value.experimental_setting.input_fields,
        ),
      );
  ...
  validateObject(withSummary: boolean) {
    ...
    this.apiService.empty_pgmMask.value.experimental_setting.input_fields.filter(
      (e) => e.position == 'experimental_setting:setting_id',
    )[0].value = 'exp' + String(this.getExpSettingID());
    if (!withSummary) {
      this.apiService
        .validateObject(this.apiService.empty_pgmMask.value, this.all_factors)
        .then((res: any) => {
          this.object_errors = res.errors;
          this.object_errors.summed =
            this.object_errors.project.length +
            this.object_errors.experimental_setting.length +
            this.object_errors.technical_details.length;
          this.object_warnings = res.warnings;
          this.object_warnings.summed =
            this.object_warnings.project.length +
            this.object_warnings.experimental_setting.length +
      ...
      Promise.all([
        this.apiService.validateObject(
          this.apiService.empty_pgmMask.value,
          this.all_factors,
          true,
        ),
        this.apiService.validateObjectWithSummary(
          this.apiService.empty_pgmMask.value,
          this.all_factors,
        ),
  ...
  getExpSettingID() {
    var exp_setting_list =
      this.apiService.empty_pgmMask.value.experimental_setting.list_value;
    var exp_setting_max_num = 0;
    for (let exp_setting of exp_setting_list) {
      var exp_setting_id = exp_setting.filter(
        (e) => e.position == 'experimental_setting:setting_id',
      )[0].value;
      if (exp_setting_id) {
  ...
  clearOrganismName(element) {
    ...
    const dialogRef = this.dialog.open(PgmAlertsComponent, {
      hasBackdrop: true,
      data: {
        title: 'Clear Organism',
        message:
          'Are you sure you want to reset the organism and reset your current input?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.apiService.empty_pgmMask.value.experimental_setting.input_fields =
          JSON.parse(JSON.stringify(this.original_exp_setting));
      }
    });
  }
  update_selected_conditions() {
    this.selected_conditions = this.conditions.filter(
      (cond) => cond.selected == true,
    );
  ...
  filterChipConditions() {
    ...
    this.selected_filter_factor_object.forEach((filter_object: any) => {
      if (filter_object.selected_filter_factor) {
        filter_object.selected_filter_factor_values.forEach(
          (filter_factor_value) => {
            filter_data = filter_data.filter((cond) =>
              cond.search.includes(
                filter_object.selected_filter_factor.factor
                  .toLowerCase()
                  .trim() +
                  ':"' +
    ...
    filter_data = filter_data.filter((cond) =>
      cond.search.some((search_element) =>
        search_element.includes(this.condition_filter_str.toLowerCase().trim()),
      ),
    );
  ...
  openFactorsDialog() {
    const dialogRef = this.dialog.open(PgmAlertsComponent, {
      hasBackdrop: true,
      data: {
        title: 'Edit factors and generate conditions',
        message:
          'Are you sure you want to edit the facors? This will reset your current input for the conditions.',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.apiService
          .getFactors(
            this.apiService.empty_pgmMask.value.experimental_setting.input_fields.filter(
              (e) => e.position == 'experimental_setting:organism',
            )[0].value,
          )
          .then((data_res: any) => {
            loadingRef.close();
            ...
            dialogRef.afterClosed().subscribe((result) => {
              if (result) {
                ...
                this.apiService
                  ...
                  .then((res_conditions: any) => {
                    console.log('factors?', this.factors);
                    this.conditions = JSON.parse(
                      JSON.stringify(res_conditions.conditions),
                    );

                    // thiis goes to filter function
                    //this.del_conditions = []
                    this.condition_whitelists[res_conditions.organism] =
                      JSON.parse(
                        JSON.stringify(res_conditions.whitelist_object),
                      );
                    console.log('whitelist test', this.condition_whitelists);
                    console.log('All conditions', this.conditions);

                    this.chip_conditions = this.conditions.map((v) =>
  ...
  getFilterFactorValues(filter_object) {
    ...
    if (filter_object.selected_filter_factor.values) {
      if (typeof filter_object.selected_filter_factor.values[0] == 'object') {
        ...
        Object.entries(filter_object.selected_filter_factor.values[0]).forEach(
          ([nested_factor, nested_value]) => {
  ...
  checkEnteredSamples() {
    var sample_counts = this.selected_conditions.map(
      (condition) => condition.list_value.length,
    );
  ...
  addExperimentalSettingDialogs() {
    const dialogRef = this.dialog.open(PgmAlertsComponent, {
      ...
      data: {
        ...
        message:
          'Are you sure you want to add the experimental setting? You will only be able to add more samples in the given conditions.',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        if (!this.checkEnteredSamples()) {
          const seconddialogRef = this.dialog.open(PgmAlertsComponent, {
            ...
            data: {
              ...
              message:
                'Please be aware that not all conditions have associated samples. Would you like to proceed and save this experimental setting regardless?',
            },
          });
  ...
  addExperimantalSetting() {
    ...
    this.apiService.empty_pgmMask.value.experimental_setting.input_fields.filter(
      (e) => e.position == 'experimental_setting:conditions',
    )[0].list_value = JSON.parse(JSON.stringify(this.selected_conditions));
    this.apiService.empty_pgmMask.value.experimental_setting.list_value.push(
      JSON.parse(
        JSON.stringify(
          this.apiService.empty_pgmMask.value.experimental_setting.input_fields,
        ),
  ...
  removeExperimentalSetting(setting) {
    this.apiService.empty_pgmMask.value.experimental_setting.list_value.splice(
      this.apiService.empty_pgmMask.value.experimental_setting.list_value.indexOf(
        setting,
      ),
      1,
    );
  ...
  requiredFieldFilledOut() {
    ...
    this.apiService.empty_pgmMask.value.project.input_fields.forEach(
      (element) => {
    ...
    var technique_elemts =
      this.apiService.empty_pgmMask.value.technical_details.input_fields.filter(
        (e) => e.position == 'technical_details:techniques',
      )[0].list_value;
    console.log(
      'technique_elemts',
      technique_elemts.length,
      'exp_setting_elements',
      exp_setting_elements.length,
    );
    if (
      technique_elemts.length != 0 &&
      technique_elemts.length >= exp_setting_elements.length
    ) {
    ...
    return [
      !fieldsFilled_project.includes(false),
      !fieldsFilled_experiment.includes(false),
      !fieldsFilled_techDetails.includes(false),
    ];
  }
  ...
  openSpecificExpandables(element_position) {
    var expaned = false;
    if (
      element_position == 'experimental_setting:experimental_factors' &&
      this.organism_was_selected
    ) {
      expaned = true;
    } else if (
      element_position == 'experimental_setting:conditions' &&
      this.selected_conditions.length > 0
    ) {
      expaned = true;
    }
