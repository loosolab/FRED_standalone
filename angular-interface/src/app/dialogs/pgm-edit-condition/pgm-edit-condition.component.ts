...
export class PgmEditConditionComponent implements OnInit {
    ...
    testIfDisabledInputAdded() {
        var elements_to_test = this.sample.filter(e => e.title && e.list && e.input_disabled)
    ...
    updateSampleNames() {
        this.sample_ls.forEach((sample, i) => {
            var sample_name = sample.filter(input => input.position == "experimental_setting:conditions:biological_replicates:samples:sample_name")[0].value
            console.log(Number(sample_name.split("_")[sample_name.split("_").length - 1]))
            if (!isNaN(Number(sample_name.split("_")[sample_name.split("_").length - 1]))) {
                var sample_name_as_ls = sample_name.split("_")
                sample_name_as_ls.pop()
                sample_name = sample_name_as_ls.join("_")
            }
            sample.filter(input => input.position == "experimental_setting:conditions:biological_replicates:samples:sample_name")[0].value = sample_name + "_" + String(i + 1)
        })
    }
