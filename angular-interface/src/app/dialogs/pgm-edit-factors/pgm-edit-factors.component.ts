...
export class PgmEditFactorsComponent implements OnInit {
  ...
  applySearchFilter(search_str: string, search_info: any){
    ...
    this.apiService.searchWhitelistElements(search_str, 25, search_info).then((res: any) => {
      this.filter_list = res.filter(single_res => !this.selected_value.some(elem => elem === single_res))
      return
    })
  }
  ...
  addFactorDisabled() {
    if (this.selected_factor) {
      if (this.data.values[this.selected_factor].input_type == 'value_unit') {
        if (this.selected_value_unit.unit != "" && this.selected_value_unit.value != "") {
          return false
        } else {
          return true
        }

      } else if (this.data.values[this.selected_factor].input_type == 'nested') {
        ...
        this.data.values[this.selected_factor].whitelist.forEach(element => {
          if (element.required) {
            //input types!          
            if (element.input_type == 'select' || element.input_type == "group_select" || element.input_type == "value_unit") {
  ...
  addFactor() {
    ...
    if (this.data.values[this.selected_factor].input_type == 'value_unit') {
      ...
      this.selected_value = value_list.filter(v => !isNaN(Number(v))).map(v => v.trim() + this.selected_value_unit.unit)
      console.log(this.selected_value)
    }
    if (this.data.values[this.selected_factor].input_type == 'nested') {
      this.selected_value.push(this.selected_value_nested)
    }
    if (this.factorList.filter(v => v.factor == this.selected_factor).length == 0) {
      //if header -> { value: "", unit: "" , header: "" }
      var factor_element = { factor: this.selected_factor, values: this.selected_value }
      if (this.data.values[this.selected_factor].headers) {
        Object.assign(factor_element, { headers: this.data.values[this.selected_factor].headers })
      }
      if (this.data.values[this.selected_factor].whitelist_keys) {
        Object.assign(factor_element, { whitelist_keys: this.data.values[this.selected_factor].whitelist_keys })
      }
      if (this.data.values[this.selected_factor].nested_infos) {
        Object.assign(factor_element, { nested_infos: this.data.values[this.selected_factor].nested_infos })
      }
      ...
      this.openSnackBar("Factor already added. If you want to edit the Factor, delete the one in the Table below.")
    }
  ...
  nestedValuesSelect(element, input_value?) {
    ...
    if (element.input_type == "select" || element.input_type == "group_select") {
      ...
      value_list = value_list.filter(v => v != "")
      this.selected_value_nested[element.position] = value_list.filter(v => !isNaN(Number(v))).map(v => v + element.unit)
  ...
  applyNestedSearchFilter(search_str: string, search_info: any, element){
    console.log("Search string:", search_str)
    this.apiService.searchWhitelistElements(search_str, 25, search_info).then((res: any) => {
      this.filter_list = res.filter(single_res => !this.selected_value_nested[element.position].some(elem => elem === single_res))
      return
    })
  }
  ...
  editFactor(element) {
    ...
    if (this.data.values[element.factor].input_type == 'select' || this.data.values[element.factor].input_type == 'multi_autofill' || this.data.values[element.factor].input_type == 'group_select' || this.data.values[element.factor].input_type == 'gene' || this.data.values[element.factor].input_type == 'enrichment') {
    ...
    if (this.data.values[element.factor].input_type == 'nested') {
      ...
      this.data.values[element.factor].whitelist.forEach(input_field => {
        console.log(input_field, this.selected_value_nested, element.values[0][input_field.position])
        if (input_field.input_type == "select" || input_field.input_type == "group_select") {
        ...
        if (input_field.input_type == "multi_autofill"){
          console.log("autofill", input_field, element.values[0][input_field.position])
  ...
  displayNested(element){
    ...
    this.data.values[element.factor].whitelist.forEach(factor => {
      if (element.values[0][factor.position].length > 0){
        displayArray.push({name: factor.displayName, value: element.values[0][factor.position]})
      }
    })
    return displayArray
  }
